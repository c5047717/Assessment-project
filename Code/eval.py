...
def main(args):
    cfg = load_cfg(args.config); set_seed(cfg['seed']); device = get_device(cfg['device'])
    dl_train, dl_val = make_loaders(cfg); vocab_size = len(dl_train.dataset.vocab)
    model = MMSequenceModel(cfg, vocab_size=vocab_size).to(device)
    ckpt_dir = cfg['log']['ckpt_dir']; ckpts = sorted([f for f in os.listdir(ckpt_dir) if f.endswith('.pt')])
    if ckpts:
        state = torch.load(os.path.join(ckpt_dir, ckpts[-1]), map_location=device)
        model.load_state_dict(state['model']); print(f"Loaded {ckpts[-1]}")
    model.eval(); ensure_dir(cfg['log']['sample_dir'])
    with torch.no_grad():
        ...
        preds = cap_logits.argmax(-1); mask = (cap_tgt != 0).float()
