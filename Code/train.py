...
def load_cfg(path): 
    with open(path,'r') as f: import yaml; return yaml.safe_load(f)
def main(args):
    cfg = load_cfg(args.config)
    for k in ['lr','weight_decay','grad_clip']: cfg['training'][k]=float(cfg['training'][k])
    ...
    for epoch in range(1, int(cfg['training']['epochs'])+1):
        model.train(); meter=AverageMeter(); pbar=tqdm(dl_train, desc=f"Train {epoch}")
        for imgs, caps, img_tgt, cap_tgt in pbar:
            ...
            if pred_latent is not None:
                with torch.no_grad(): true_latent = model.ae.encode(img_tgt)
                total = total + cfg['loss'].get('latent_mse_weight', 0.5)*F.mse_loss(pred_latent, true_latent)
            opt.zero_grad(); total.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), cfg['training']['grad_clip'])
            opt.step()
            meter.update(total.item(), imgs.size(0)); pbar.set_postfix(loss=f"{meter.avg:.3f}")
        ...
        with torch.no_grad():
            for imgs,caps,img_tgt,cap_tgt in dl_val:
                ...
                if pred_latent is not None:
                    ...
                    loss = loss + cfg['loss'].get('latent_mse_weight', 0.5)*F.mse_loss(pred_latent, true_latent)
